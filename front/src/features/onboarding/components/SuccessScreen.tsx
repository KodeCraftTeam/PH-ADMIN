...
function Confetti() {
  // 60 piezas con posición/duración/retraso aleatorios, generadas una sola vez
  // al montar (useState con inicializador, no useMemo — es el lugar correcto
  // para cálculo impuro de una sola vez según las reglas de pureza de React).
...
export function SuccessScreen() {
  const { property, propertyId, importPreview } = useWizardState();
  ...
  return (
    <div className="flex flex-col items-center px-4 py-16 text-center">
      ...
      <h1 className="mt-6 text-2xl sm:text-3xl font-bold text-slate-900 dark:text-zinc-100">
        ¡Tu conjunto está listo!
      </h1>
      <p className="mt-3 max-w-md text-slate-500 dark:text-zinc-400">
      ...
      <div className="mt-8 flex w-full max-w-xs flex-col gap-3 sm:max-w-none sm:w-auto sm:flex-row">
        <Button onClick={() => router.push(propertyId ? `/admin/${propertyId}` : "/admin")}>
